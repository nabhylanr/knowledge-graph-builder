...
class GateConfig(BaseModel):
    ...
    # 120s, not 30s: a reasoning model (e.g. qwen3:4b) emits a thinking block
...
class ClassificationConfig(BaseModel):
    ...
    Unlike gates (local dev model, fail-open, no retry), this stage uses a
    bigger, more capable model than gating/extraction — Ollama by default
    (`model_type="ollama"`), Groq still supported via `model_type="groq"` for
    a real hosted API with real rate limits. Retry conventions mirror
    ...
    why); the 429/rate-limit branch is a no-op against Ollama but harmless.
    """
